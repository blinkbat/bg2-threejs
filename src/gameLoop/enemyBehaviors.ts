...
/**
 * Check if a kraken is currently submerged.
 */
...
/**
 * Update submerged krakens - handle resurfacing.
 */
export function updateSubmergedKrakens(
    ...
    for (let i = submergedKrakens.length - 1; i >= 0; i--) {
        ...
        // Check if submerge duration expired
        if (now >= sk.submergeEndTime) {
            const krakenG = unitsRef[sk.unitId];
            if (krakenG) {
                krakenG.visible = true;
            }
            addLog("The Baby Kraken resurfaces!", "#6b3fa0");
            soundFns.playSplash();
            submergedKrakens.splice(i, 1);
        }
    }
